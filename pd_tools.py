...
def df_diff(df_new: pd.DataFrame, df_old: pd.DataFrame, key_columns: list) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Find difference between two DataFrames by key columns.
    Returns (new_data, updated_data).
    - new_data: rows present in df_new but not in df_old
    - updated_data: rows where key exists in both but some values differ
    """
    ...
    common_cols = [c.replace("_new", "") for c in new_cols if c.replace("_new", "_old") in old_cols]

    diff_mask = (
        merged[[c + "_new" for c in common_cols]].values
        != merged[[c + "_old" for c in common_cols]].values
    ).any(axis=1)
